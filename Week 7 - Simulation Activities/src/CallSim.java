...
public class CallSim {
     ...
     * next DIAL_IN event will occur. In practice, we would use a random number to
     * set the time.
     */
    ...
    /**
     * Run the simulation until stoppingTime occurs. Print output as in Figure 13.4.
     */
	...
	while (!eventSet.isEmpty()) {
		...
		if (availableOperators > 0) {
		    availableOperators--;
		    howLong = nextPoisson(avgCallLen);
	...
	double product = Math.log(r.nextDouble());
	int count;

	for (count = 0; product > limit; count++)
	    product += Math.log(r.nextDouble());
